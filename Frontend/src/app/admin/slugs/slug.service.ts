...
export class SlugService {
  ...
  get slugsSnapshot(): SlugEntry[] {
    return this.slugsSubject.value;
  }
  get selectedSlugSnapshot(): SlugEntry | null {
    return this.selectedSlugSubject.value;
  }

  loadSlugs(): Observable<SlugEntry[]> {
    return this.http
      .get<SlugEntry[]>(this.api.buildUrl('slugs/'))
  ...
  createSlug(slug: string): Observable<SlugEntry | null> {
    return this.http.post<SlugEntry>(this.api.buildUrl('auth/create_slug/'), { slug }).pipe(
  ...
  updateSlug(id: number, newSlug: string): Observable<SlugEntry | null> {
    return this.http
      .put<SlugEntry>(this.api.buildUrl(`auth/alter_slug/${id}`), { slug: newSlug })
      .pipe(
        tap((updated) => {
  ...
  deleteSlug(id: number): Observable<boolean> {
    return this.http.delete(this.api.buildUrl(`auth/alter_slug/${id}`)).pipe(
      tap(() => {
