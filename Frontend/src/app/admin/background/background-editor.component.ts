// src/app/admin/background/background-editor.component.ts
import { Component, OnDestroy, OnInit, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import {
  ReactiveFormsModule, FormBuilder, Validators,
  FormGroup, FormControl, ValidatorFn, AbstractControl, ValidationErrors
} from '@angular/forms';
...
@Component({
    ...
    <form [formGroup]="form" (ngSubmit)="save()">
      <div class="grid">
        ...
        <mat-form-field appearance="fill">
          ...
          <mat-error *ngIf="form.controls['arrowColor'].hasError('pattern')">Invalid hex</mat-error>
        </mat-form-field>
      ...
      <div class="actions">
        ...
        <button mat-button type="button" (click)="resetToServer()" [disabled]="loading || !hasRecord">Reset</button>
        <button mat-button color="warn" type="button" (click)="confirmDelete()" [disabled]="deleting || !hasRecord">
          Delete
        </button>
      </div>
    </form>
  ...
  `,
  styles: [`
    .bg-card { max-width: 1000px; margin: 16px auto; padding: 16px; }
    .header { display:flex; align-items:center; gap: 16px; }
    .header .owner { margin-left: auto; }
    .grid {
      display: grid;
...
export class BackgroundEditorComponent implements OnInit, OnDestroy {
    ...
    saving = false;
    deleting = false;   
    get hasRecord() { return !!this.record; }   
...
/** ✅ Properly typed custom validator */
function hexValidator(): ValidatorFn {
  const HEX = /^#(?:[0-9a-fA-F]{3}){1,2}$/;
  return (control: AbstractControl): ValidationErrors | null => {
    const v = control.value as string;
    if (!v) return null;               // allow empty; backend can enforce if required
