...
export class BackgroundService {
  ...
  getByOwner(ownerSlug: string): Observable<BackgroundData | null> {
    const gradient$ = this.http
      .get<any>(this.api.buildUrl(this.GRADIENT_URL(ownerSlug)))
      .pipe(catchError(() => of(null)));
    const details$ = this.http
      .get<any>(this.api.buildUrl(this.DETAILS_URL(ownerSlug)))
      .pipe(catchError(() => of(null)));
    const names$ = this.http
      .get<any>(this.api.buildUrl(this.NAMES_URL(ownerSlug)))
      .pipe(catchError(() => of(null)));
    ...
    return forkJoin([gradient$, details$, names$]).pipe(
      map(([g, d, n]) => {
        ...
        return {
          ...
          position2: g?.position2 ?? '',
          position3: g?.position3 ?? '',
          page1: n?.page1 ?? '',
          page2: n?.page2 ?? '',
          navColor: d?.navColor ?? '#000000',
          arrowColor: d?.arrowColor ?? '#FFFFFF',
          ellipseWidth: d?.ellipseWidth ?? 0,
          ellipseHeight: d?.ellipseHeight ?? 0,
          created_at,
          edited_at,
        } as BackgroundData;
      }),
  ...
  createBackground(
    owner: string,
    values: Omit<BackgroundData, 'id' | 'created_at' | 'edited_at'>,
  ): Observable<BackgroundData | null> {
    const body = { owner, ...values };
    return this.http.post<BackgroundData>(this.api.buildUrl(this.CREATE_URL), body).pipe(
      catchError((err) => {
  ...
  updateBackground(
    id: number,
    owner: string,
    patch: Partial<Omit<BackgroundData, 'id' | 'created_at' | 'edited_at'>>,
  ): Observable<BackgroundData | null> {
    const body = { owner, ...patch };
    return this.http.put<BackgroundData>(this.api.buildUrl(this.ALTER_URL(id)), body).pipe(
      catchError((err) => {
        console.error('Update background failed', err);
        return of(null);
      }),
    );
  }
  ...
  deleteBackground(id: number): Observable<boolean> {
    return this.http.delete(this.api.buildUrl(this.ALTER_URL(id))).pipe(
      map(() => true),
      catchError((err) => {
