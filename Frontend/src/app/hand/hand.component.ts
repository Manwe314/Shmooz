...
export class HandComponent {
  ...
  constructor(
    ...
    private handState: HandStateService,
  ) {}
  ...
  computeArgument(index: number, total: number): number {
    ...
    if (total % 2 === 1) {
      ...
      const half = total / 2;
      if (index < half) return -(half - index) * step_angle + step_angle / 2;
      else return (index - half + 1) * step_angle - step_angle / 2;
    }
  }
  ...
  getCardTransform(index: number, total: number, card: ProjectCard): string {
    ...
    const rotation =
      Math.atan2(vertiacalRadius * Math.sin(argument), horizontalRadius * Math.cos(argument)) *
      (180 / Math.PI);
