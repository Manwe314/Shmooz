from drf_spectacular.utils import OpenApiTypes, OpenApiExample, OpenApiResponse, extend_schema_field
...
class ImageContentSchema(BaseContentSchema):
    ...
    objectFit = serializers.ChoiceField(choices=['cover', 'contain', 'fill', 'none', 'scale-down'], required=False)
    objectPosition = serializers.CharField(required=False)
    overflow = serializers.ChoiceField(choices=['visible', 'hidden', 'scroll'], required=False)
...
class TextContentSchema(BaseContentSchema):
    ...
    tag = serializers.ChoiceField(choices=['p', 'h1', 'h2', 'span', 'div'], required=False)
    horizontalAlign = serializers.ChoiceField(choices=['left', 'center', 'right'], required=False)
    verticalAlign = serializers.ChoiceField(choices=['top', 'center', 'bottom'], required=False)
    textAlign = serializers.ChoiceField(choices=['left', 'center', 'right'], required=False)
...
class BlockSchema(serializers.Serializer):
    ...
    content = serializers.ListField(
        child=BlockContentSchema,
        help_text="List of image/text/link elements"
    )

class PageSchema(serializers.Serializer):
    content = serializers.ListField(
        child=BlockSchema(),
        help_text="List of blocks for the page layout"
    )
